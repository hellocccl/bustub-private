...
auto BPLUSTREE_TYPE::PessimisticSearch(const KeyType &key, SearchMode mode, Transaction *transaction,
  ...
  const auto smart_use = [this, transaction, latched_pages, use_mode](page_id_t page_id) -> Page * {
    Page *page;
    page = UsePage(page_id, use_mode, transaction);
    latched_pages->push_back(page);
    return page;
  };
  const auto reset_latched_pages = [this, latched_pages, use_mode](BPlusTreePage *tree_page) {
